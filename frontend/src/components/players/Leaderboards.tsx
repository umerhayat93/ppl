...
export default function Leaderboards() {
  ...
  // Top 10 batters by total runs — players.team comes from DB join (always correct)
  ...
  if (!topBat.length && !topBowl.length) return (
    <div className="mx-3 py-6 text-center text-[#4a5568] text-xs card mt-2 mb-4">
      📊 Leaderboards will appear after matches are scored
    </div>
  )
            ...
            {topBat.map((p, i) => (
              <div key={p.id} className="flex items-center gap-3 px-4 py-3">
                <span className={`w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-bold flex-shrink-0
                  ${i===0?'bg-[#f0c040]/20 text-[#f0c040]':i===1?'bg-gray-400/20 text-gray-300':i===2?'bg-amber-700/30 text-amber-600':'bg-white/[0.03] text-[#4a5568]'}`}>
                  {i + 1}
                </span>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {/* team comes from DB join — always correct */}
                ...
                <div className="text-right flex-shrink-0">
                  <div className="font-mono font-black text-[#f0c040] text-2xl leading-none">{p.runs}</div>
                  <div className="text-[9px] text-[#4a5568] uppercase tracking-widest">runs</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mx-3 card overflow-hidden">
          ...
          <div className="divide-y divide-white/[0.03]">
            {topBowl.map((p, i) => (
              <div key={p.id} className="flex items-center gap-3 px-4 py-3">
                <span className={`w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-bold flex-shrink-0
                  ${i===0?'bg-red-500/20 text-red-400':i===1?'bg-gray-400/20 text-gray-300':i===2?'bg-amber-700/30 text-amber-600':'bg-white/[0.03] text-[#4a5568]'}`}>
                  {i + 1}
                </span>
                <span className="text-xl leading-none flex-shrink-0">{p.emoji || '🏏'}</span>
                <div className="flex-1 min-w-0">
                ...
                <div className="text-right flex-shrink-0">
                  <div className="font-mono font-black text-red-400 text-2xl leading-none">{p.wickets}W</div>
                  <div className="text-[9px] text-[#4a5568] uppercase tracking-widest">wickets</div>
                </div>
              </div>
            ))}
          </div>
