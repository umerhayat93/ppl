...
export default function MatchesSection() {
  ...
  const save = async () => {
    ...
    try {
      ...
      setForm(INIT); setEditId(''); setShowForm(false)
      await reload()
  ...
  const del = async (id: string) => {
    ...
    try { await api('/matches/' + id, 'DELETE'); await reload() }
    catch (e: any) { toast.error(e.message) }
  }

  const edit = (m: any) => {
    setEditId(m.id)
    setForm({ stage:m.stage, groupId:m.groupId||'', matchNo:m.matchNo, team1Id:m.team1Id, team2Id:m.team2Id, date:m.date, time:m.time, year:m.year, venue:m.venue, status:m.status, result:m.result, score1:m.score1, score2:m.score2, overs:m.overs })
    setShowForm(true)
  }
  ...
  return (
    <div className="space-y-3">
      <button onClick={() => { setShowForm(s => !s); setEditId(''); setForm(INIT) }} className="btn-gold w-full">
      ...
      {showForm && (
        <div className="space-y-2 card p-3">
          ...
          {needsResult && (
            <>
              <input className="input-field" placeholder="Result e.g. Team A won by 5 wickets" value={form.result} onChange={e => set('result', e.target.value)} />
              {form.status === 'completed' && (
                <div className="grid grid-cols-2 gap-2">
                  <input className="input-field" placeholder="Score 1 e.g. 120/4 (10)" value={form.score1} onChange={e => set('score1', e.target.value)} />
                  <input className="input-field" placeholder="Score 2 e.g. 115/8 (10)" value={form.score2} onChange={e => set('score2', e.target.value)} />
                </div>
              )}
            </>
          )}
